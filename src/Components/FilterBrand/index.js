...
const FilterBrand = () => {
    ...
    const itemNameValue = brand_name ? brand_name : "";
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // First API call
                const response1 = await axios.get(`http://localhost:8000/api/carousel/all/filter?brand_name=${itemNameValue}`);
                setNewUpdatedBanner(response1.data);
                setLoading(false)
                console.log("updateBanner", response1.data)

                // Second API call
                const response2 = await axios.get(`http://localhost:8000/api/products/all/filter?brand_name=${itemNameValue}`);
                                ...
                                <div className='carousel-main-container'>
                                    <Slider {...settings}>
                                        {newUpdatedBanner.map((eachData, index) => (
                                            <div className="carousel-item1" key={eachData.id} onClick={() => alert(`clicked`)}>
